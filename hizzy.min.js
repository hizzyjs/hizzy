...
module.exports = class MyAddon extends AddonModule {
  ...
  "version": "1.0.0",
  "main": "index.js"
}`),printer.dev.pass("A new addon has been set up at '%c"+s+"&t'!","color: orange"),process.exit()}if(_argv_.build||_argv_["no-clear"]||(printer.clear(),printer.print("\n")),isTerminal&&args[0]){const t=path.join(process.cwd(),args[0]);fs.existsSync(t)||exit("Couldn't open the file %c'"+t+"'%c.","color: orange","color: red"),dir=fs.statSync(t).isDirectory()?t:(file=t,path.dirname(t))}(async()=>{var e={dev:!0,port:1881,fileRefresh:!0,autoBuild:!0,listen:!0,main:_argv_.ts?"Server.tsx":"Server.jsx",mainModule:!0,checkConfig:!0,realtime:!0,static:{},https:!1,srcFolder:"src",connectionTimeout:6e4,keepaliveTimeout:3e4,clientKeepalive:2e4,minClientKeepalive:8e3,addons:[],includeOriginalInBuild:!0,cache:{addons:31536e3,npm:31536e3,preact:31536e3,"preact-hooks":31536e3,"html-injection":31536e3,"jsx-injection":31536e3,static:{}},warnAboutTypes:!0};let r=_argv_.config;if(r&&!fs.existsSync(path.join(dir,r)))return exit("Config file %c"+r+"&t given in the command line arguments is invalid:","color: orange");var o=["json","config.json","config.ts","config.js"];if(_argv_.ts?o.push("config.mjs","config.mts"):o.push("config.mts","config.mjs"),!r)for(const g of o)if(r=__PRODUCT__+"."+g,fs.existsSync(path.join(dir,r)))break;var o=path.join(dir,r),t=fs.existsSync(o);t&&fs.statSync(o).isFile()||(t&&fs.rmSync(o),_argv_.debug&&printer.dev.debug("Creating the %c/"+r+"&t file...","color: orange"),fs.writeFileSync(o,`export default Hizzy.defineConfig({${_argv_.ts?`
    main: "Server.tsx"
`:""}});`));let i;if(r.endsWith(".json"))try{i=JSON.parse(fs.readFileSync(o,"utf8"))}catch(e){return exit("Invalid JSON in the file %c'"+o+"'%c: %c"+e.toString()+"%c.","color: orange","color: red","color: orange","color: red")}else if(r.endsWith(".js")||r.endsWith(".mjs")||r.endsWith(".ts")||r.endsWith(".mts"))if(global[__PRODUCT_U__]={defineConfig:e=>e||{}},r.endsWith(".js")||r.endsWith(".mjs"))i=(await import(url.pathToFileURL(o))).default;else{t=path.join(path.dirname(o),random()+"."+(r.endsWith(".mts")?"m":"")+"js");fs.writeFileSync(t,require("@babel/core").transformSync(fs.readFileSync(o),{filename:path.basename(o),presets:[require("@babel/preset-typescript")]}).code);try{i=(await import(url.pathToFileURL(t))).default}finally{fs.rmSync(t)}}"function"==typeof i&&await i({argv:_argv_,isDev:!!_argv_.dev});var t=i.checkConfig,n=checkDefault(i,e);if(!_argv_.build&&n.length&&t&&r.endsWith(".json")&&(fs.writeFileSync(o,JSON.stringify(i,null,2)),printer.dev.warn("Updated following properties in "+__PRODUCT__+".json: "+n.map(e=>e.join("->")).join(", "))),Array.isArray(i.extensionRemovals)||(i.extensionRemovals=e.extensionRemovals),i.extensionRemovals=[...new Set(i.extensionRemovals)],-1!==i.keepaliveTimeout&&i.clientKeepalive>=i.keepaliveTimeout)return exit("Config's 'clientKeepalive' property has to be smaller than 'keepaliveTimeout'.");if(Array.isArray(i.static)){t=i.static;i.static={};for(const p of t)i.static[p]=p}if(Object.freeze(i),(self.config=i).warnAboutTypes)try{require(dir+"/node_modules/@hizzyjs/types");var s=require(dir+"/node_modules/@hizzyjs/types/package.json")["version"],a=require("./package.json").version;s!==a&&printer.dev.warn(`The installed %c@hizzyjs/types@${s}&t's version don't match the version of%c hizzy@${a}&t, please consider updating. %cYou can disable this by setting 'warnAboutTypes' to false in config file.`,"color: orange","color: orange","color: gray")}catch(e){}_argv_.debug||(printer.options.disabledTags.push("debug"),printer.dev.options.disabledTags.push("debug"));o=fs.existsSync(path.join(dir,".build")),n=path.join(dir,i.srcFolder),e=fs.existsSync(n),e&&fs.statSync(n).isDirectory()||(o?printer.dev.debug("Skipping the creation of %c/"+i.srcFolder+"&t because there is an existing build.","color: orange"):(e&&fs.rmSync(n),printer.dev.debug("Creating the %c/"+i.srcFolder+"&t folder...","color: orange"),fs.mkdirSync(n),fs.writeFileSync(path.join(n,"App.jsx"),`const foo = 20;
